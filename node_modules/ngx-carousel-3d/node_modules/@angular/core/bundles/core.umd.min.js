 ...
 * found in the LICENSE file at https://angular.io/license
 */
function assertEqual(actual,expected,msg){actual!=expected&&throwError(msg)}function assertNotEqual(actual,expected,msg){actual==expected&&throwError(msg)}function assertSame(actual,expected,msg){actual!==expected&&throwError(msg)}function assertLessThan(actual,expected,msg){actual>=expected&&throwError(msg)}function assertNull(actual,msg){null!=actual&&throwError(msg)}function assertNotNull(actual,msg){null==actual&&throwError(msg)}function throwError(msg){throw new Error("ASSERTION ERROR: "+msg)}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
function queueInitHooks(index,onInit,doCheck,tView){!0===tView.firstTemplatePass&&(null!=onInit&&(tView.initHooks||(tView.initHooks=[])).push(index,onInit),null!=doCheck&&((tView.initHooks||(tView.initHooks=[])).push(index,doCheck),(tView.checkHooks||(tView.checkHooks=[])).push(index,doCheck)))}function queueLifecycleHooks(flags,currentView){var tView=currentView.tView;if(!0===tView.firstTemplatePass)for(var size=(4092&flags)>>2,start=flags>>12,i=start,end=start+size;i<end;i++){var def=tView.data[i];queueContentHooks(def,tView,i),queueViewHooks(def,tView,i),queueDestroyHooks(def,tView,i)}}function queueContentHooks(def,tView,i){null!=def.afterContentInit&&(tView.contentHooks||(tView.contentHooks=[])).push(i,def.afterContentInit),null!=def.afterContentChecked&&((tView.contentHooks||(tView.contentHooks=[])).push(i,def.afterContentChecked),(tView.contentCheckHooks||(tView.contentCheckHooks=[])).push(i,def.afterContentChecked))}function queueViewHooks(def,tView,i){null!=def.afterViewInit&&(tView.viewHooks||(tView.viewHooks=[])).push(i,def.afterViewInit),null!=def.afterViewChecked&&((tView.viewHooks||(tView.viewHooks=[])).push(i,def.afterViewChecked),(tView.viewCheckHooks||(tView.viewCheckHooks=[])).push(i,def.afterViewChecked))}function queueDestroyHooks(def,tView,i){null!=def.onDestroy&&(tView.destroyHooks||(tView.destroyHooks=[])).push(i,def.onDestroy)}function executeInitHooks(currentView,tView,creationMode){1===currentView.lifecycleStage&&(executeHooks(currentView.data,tView.initHooks,tView.checkHooks,creationMode),currentView.lifecycleStage=2)}function executeContentHooks(currentView,tView,creationMode){currentView.lifecycleStage<3&&(executeHooks(currentView.data,tView.contentHooks,tView.contentCheckHooks,creationMode),currentView.lifecycleStage=3)}function executeHooks(data,allHooks,checkHooks,creationMode){var hooksToCall=creationMode?allHooks:checkHooks;null!=hooksToCall&&callHooks(data,hooksToCall)}function callHooks(data,arr){for(var i=0;i<arr.length;i+=2)arr[1|i].call(data[arr[i]])}function isProceduralRenderer(renderer){return!!renderer.listen}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
function findNextRNodeSibling(node,stopNode){for(var currentNode=node;currentNode&&currentNode!==stopNode;){var pNextOrParent=currentNode.pNextOrParent;if(pNextOrParent){for(var pNextOrParentType=3&pNextOrParent.flags;1!==pNextOrParentType;){var nativeNode=findFirstRNode(pNextOrParent);if(nativeNode)return nativeNode;pNextOrParent=pNextOrParent.pNextOrParent}currentNode=pNextOrParent}else{for(var currentSibling=currentNode.next;currentSibling;){var nativeNode=findFirstRNode(currentSibling);if(nativeNode)return nativeNode;currentSibling=currentSibling.next}var parentNode=currentNode.parent;if(currentNode=null,parentNode){var parentType=3&parentNode.flags;0!==parentType&&2!==parentType||(currentNode=parentNode)}}}return null}function getNextLNodeWithProjection(node){var pNextOrParent=node.pNextOrParent;if(pNextOrParent){return 1==(3&pNextOrParent.flags)?null:pNextOrParent}return node.next}function getNextOrParentSiblingNode(initialNode,rootNode){for(var node=initialNode,nextNode=getNextLNodeWithProjection(node);node&&!nextNode;){if((node=node.pNextOrParent||node.parent)===rootNode)return null;nextNode=node&&getNextLNodeWithProjection(node)}return nextNode}function findFirstRNode(rootNode){for(var node=rootNode;node;){var type=3&node.flags,nextNode=null;if(3===type)return node.native;if(0===type){var childContainerData=node.data;nextNode=childContainerData.views.length?childContainerData.views[0].child:null}else nextNode=1===type?node.data.head:node.child;node=null===nextNode?getNextOrParentSiblingNode(node,rootNode):nextNode}return null}function addRemoveViewFromContainer(container,rootNode,insertMode,beforeNode){ngDevMode&&assertNodeType(container,0),ngDevMode&&assertNodeType(rootNode,2);var parentNode=container.data.renderParent,parent=parentNode?parentNode.native:null,node=rootNode.child;if(parent)for(;node;){var type=3&node.flags,nextNode=null,renderer=container.view.renderer;if(3===type)insertMode?isProceduralRenderer(renderer)?renderer.insertBefore(parent,node.native,beforeNode):parent.insertBefore(node.native,beforeNode,!0):isProceduralRenderer(renderer)?renderer.removeChild(parent,node.native):parent.removeChild(node.native),nextNode=node.next;else if(0===type){var childContainerData=node.data;childContainerData.renderParent=parentNode,nextNode=childContainerData.views.length?childContainerData.views[0].child:null}else nextNode=1===type?node.data.head:node.child;node=null===nextNode?getNextOrParentSiblingNode(node,rootNode):nextNode}}function destroyViewTree(rootView){for(var viewOrContainer=rootView;viewOrContainer;){var next=null;if(viewOrContainer.views&&viewOrContainer.views.length?next=viewOrContainer.views[0].data:viewOrContainer.child?next=viewOrContainer.child:viewOrContainer.next&&(cleanUpView(viewOrContainer),next=viewOrContainer.next),null==next){for(;viewOrContainer&&!viewOrContainer.next;)cleanUpView(viewOrContainer),viewOrContainer=getParentState(viewOrContainer,rootView);cleanUpView(viewOrContainer||rootView),next=viewOrContainer&&viewOrContainer.next}viewOrContainer=next}}function insertView(container,newView,index){var state=container.data,views=state.views;if(index>0&&setViewNext(views[index-1],newView),index<views.length&&views[index].data.id!==newView.data.id?(setViewNext(newView,views[index]),views.splice(index,0,newView)):index>=views.length&&views.push(newView),state.nextIndex<=index&&state.nextIndex++,null!==container.data.renderParent){var beforeNode=findNextRNodeSibling(newView,container);if(!beforeNode){var containerNextNativeNode=container.native;void 0===containerNextNativeNode&&(containerNextNativeNode=container.native=findNextRNodeSibling(container,null)),beforeNode=containerNextNativeNode}addRemoveViewFromContainer(container,newView,!0,beforeNode)}return newView}function removeView(container,removeIndex){var views=container.data.views,viewNode=views[removeIndex];return removeIndex>0&&setViewNext(views[removeIndex-1],viewNode.next),views.splice(removeIndex,1),destroyViewTree(viewNode.data),addRemoveViewFromContainer(container,viewNode,!1),container.data.queries&&container.data.queries.removeView(removeIndex),viewNode}function setViewNext(view,next){view.next=next,view.data.next=next?next.data:null}function getParentState(state,rootView){var node;return(node=state.node)&&2==(3&node.flags)?node.parent.data:state.parent===rootView?null:state.parent}function cleanUpView(view){removeListeners(view),executeOnDestroys(view)}function removeListeners(view){var cleanup=view.cleanup;if(null!=cleanup){for(var i=0;i<cleanup.length-1;i+=2)"string"==typeof cleanup[i]?(cleanup[i+1].removeEventListener(cleanup[i],cleanup[i+2],cleanup[i+3]),i+=2):cleanup[i].call(cleanup[i+1]);view.cleanup=null}}function executeOnDestroys(view){var destroyHooks,tView=view.tView;null!=tView&&null!=(destroyHooks=tView.destroyHooks)&&callHooks(view.data,destroyHooks)}function canInsertNativeNode(parent,currentView){return 3==(3&parent.flags)&&(parent.view!==currentView||null===parent.data)}function appendChild(parent,child,currentView){if(null!==child&&canInsertNativeNode(parent,currentView)){var renderer=currentView.renderer;return isProceduralRenderer(renderer)?renderer.appendChild(parent.native,child):parent.native.appendChild(child),!0}return!1}function insertChild(node,currentView){var parent=node.parent;if(canInsertNativeNode(parent,currentView)){var nativeSibling=findNextRNodeSibling(node,null),renderer=currentView.renderer;isProceduralRenderer(renderer)?renderer.insertBefore(parent.native,node.native,nativeSibling):parent.native.insertBefore(node.native,nativeSibling,!1)}}function appendProjectedNode(node,currentParent,currentView){if(0!=(3&node.flags))appendChild(currentParent,node.native,currentView);else if(canInsertNativeNode(currentParent,currentView)){var lContainer=node.data;lContainer.renderParent=currentParent;for(var views=lContainer.views,i=0;i<views.length;i++)addRemoveViewFromContainer(node,views[i],!0,null)}}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
function isDifferent(a,b){return!(a!==a&&b!==b)&&a!==b}function stringify$1(value){return"function"==typeof value?value.name||value:"string"==typeof value?value:null==value?"":""+value}function notImplemented(){return new Error("NotImplemented")}function flatten$1(list){for(var result=[],i=0;i<list.length;){var item=list[i];Array.isArray(item)?item.length>0?(list=item.concat(list.slice(i+1)),i=0):i++:(result.push(item),i++)}return result}function getRenderer(){return renderer}function getPreviousOrParentNode(){return previousOrParentNode}function getCurrentQueries(QueryType){return currentQueries||(currentQueries=new QueryType)}function getCreationMode(){return creationMode}function enterView(newView,host){var oldView=currentView;return data=newView&&newView.data,bindingIndex=newView&&newView.bindingStartIndex||0,tData=newView&&newView.tView.data,creationMode=newView&&newView.creationMode,cleanup=newView&&newView.cleanup,renderer=newView&&newView.renderer,null!=host&&(previousOrParentNode=host,isParent=!0),currentView=newView,currentQueries=newView&&newView.queries,oldView}function leaveView(newView){executeHooks(currentView.data,currentView.tView.viewHooks,currentView.tView.viewCheckHooks,creationMode),currentView.creationMode=!1,currentView.lifecycleStage=1,currentView.tView.firstTemplatePass=!1,enterView(newView,null)}function createLView(viewId,renderer,tView,template,context){return{parent:currentView,id:viewId,node:null,data:[],tView:tView,cleanup:null,renderer:renderer,child:null,tail:null,next:null,bindingStartIndex:null,creationMode:!0,template:template,context:context,dynamicViewCount:0,lifecycleStage:1,queries:null}}function createLNode(index,type,native,state){var parent=isParent?previousOrParentNode:previousOrParentNode&&previousOrParentNode.parent,queries=(isParent?currentQueries:previousOrParentNode&&previousOrParentNode.queries)||parent&&parent.queries&&parent.queries.child(),isState=null!=state,node={flags:type,native:native,view:currentView,parent:parent,child:null,next:null,nodeInjector:parent?parent.nodeInjector:null,data:isState?state:null,queries:queries,tNode:null,pNextOrParent:null};return 2==(2&type)&&isState&&(ngDevMode&&assertNull(state.node,"LView.node should not have been initialized"),state.node=node),null!=index&&(ngDevMode&&assertDataNext(index),data[index]=node,index>=tData.length?tData[index]=null:node.tNode=tData[index],isParent?(currentQueries=null,previousOrParentNode.view!==currentView&&2!=(3&previousOrParentNode.flags)||(ngDevMode&&assertNull(previousOrParentNode.child,"previousOrParentNode's child should not have been set."),previousOrParentNode.child=node)):previousOrParentNode&&(ngDevMode&&assertNull(previousOrParentNode.next,"previousOrParentNode's next property should not have been set."),previousOrParentNode.next=node)),previousOrParentNode=node,isParent=!0,node}function resetApplicationState(){isParent=!1,previousOrParentNode=null}function renderEmbeddedTemplate(viewNode,template,context,renderer){var _isParent=isParent,_previousOrParentNode=previousOrParentNode;try{isParent=!0,previousOrParentNode=null;var cm=!1;if(null==viewNode){viewNode=createLNode(null,2,null,createLView(-1,renderer,createTView(),template,context)),cm=!0}enterView(viewNode.data,viewNode),template(context,cm)}finally{refreshDynamicChildren(),leaveView(currentView.parent),isParent=_isParent,previousOrParentNode=_previousOrParentNode}return viewNode}function renderComponentOrTemplate(node,hostView,componentOrContext,template){var oldView=enterView(hostView,node);try{rendererFactory.begin&&rendererFactory.begin(),template?template(componentOrContext,creationMode):componentRefresh(1,0)}finally{rendererFactory.end&&rendererFactory.end(),leaveView(oldView)}}function elementStart(index,nameOrComponentType,attrs,directiveTypes,localRefs){var node,native;if(null==nameOrComponentType){var node_1=data[index];native=node_1&&node_1.native}else{ngDevMode&&assertNull(currentView.bindingStartIndex,"elements should be created before any bindings");var isHostElement="string"!=typeof nameOrComponentType,hostComponentDef=isHostElement?nameOrComponentType.ngComponentDef:null,name_1=isHostElement?hostComponentDef.tag:nameOrComponentType;if(null===name_1)throw"for now name is required";native=renderer.createElement(name_1);var componentView=null;if(isHostElement){var tView=getOrCreateTView(hostComponentDef.template);componentView=addToViewTree(createLView(-1,rendererFactory.createRenderer(native,hostComponentDef.rendererType),tView,null,null))}node=createLNode(index,3,native,componentView);var queryName=hack_findQueryName(hostComponentDef,localRefs,"");null==node.tNode&&(ngDevMode&&assertDataInRange(index-1),node.tNode=tData[index]=createTNode(name_1,attrs||null,null,hostComponentDef?null:queryName)),attrs&&setUpAttributes(native,attrs),appendChild(node.parent,native,currentView),hostComponentDef&&directiveCreate(++index,hostComponentDef.n(),hostComponentDef,queryName),hack_declareDirectives(index,directiveTypes,localRefs)}return native}function hack_declareDirectives(index,directiveTypes,localRefs){if(directiveTypes)for(var i=0;i<directiveTypes.length;i++){var directiveType=directiveTypes[i],directiveDef=directiveType.ngDirectiveDef;directiveCreate(++index,directiveDef.n(),directiveDef,hack_findQueryName(directiveDef,localRefs))}}function hack_findQueryName(directiveDef,localRefs,defaultExport){var exportAs=directiveDef&&directiveDef.exportAs||defaultExport;if(null!=exportAs&&localRefs)for(var i=0;i<localRefs.length;i+=2){var local=localRefs[i],toExportAs=localRefs[1|i];if(toExportAs===exportAs||toExportAs===defaultExport)return local}return null}function getOrCreateTView(template){return template.ngPrivateData||(template.ngPrivateData=createTView())}function createTView(){return{data:[],firstTemplatePass:!0,initHooks:null,checkHooks:null,contentHooks:null,contentCheckHooks:null,viewHooks:null,viewCheckHooks:null,destroyHooks:null}}function setUpAttributes(native,attrs){ngDevMode&&assertEqual(attrs.length%2,0,"each attribute should have a key and a value");for(var isProc=isProceduralRenderer(renderer),i=0;i<attrs.length;i+=2)isProc?renderer.setAttribute(native,attrs[i],attrs[1|i]):native.setAttribute(attrs[i],attrs[1|i])}function createError(text,token){return new Error("Renderer: "+text+" ["+stringify$1(token)+"]")}function locateHostElement(factory,elementOrSelector){ngDevMode&&assertDataInRange(-1),rendererFactory=factory;var defaultRenderer=factory.createRenderer(null,null),rNode="string"==typeof elementOrSelector?isProceduralRenderer(defaultRenderer)?defaultRenderer.selectRootElement(elementOrSelector):defaultRenderer.querySelector(elementOrSelector):elementOrSelector;if(ngDevMode&&!rNode)throw"string"==typeof elementOrSelector?createError("Host node with selector not found:",elementOrSelector):createError("Host node is required:",elementOrSelector);return rNode}function hostElement(rNode,def){resetApplicationState(),createLNode(0,3,rNode,createLView(-1,renderer,getOrCreateTView(def.template),null,null))}function listener(eventName,listener,useCapture){void 0===useCapture&&(useCapture=!1),ngDevMode&&assertPreviousIsParent();var node=previousOrParentNode,native=node.native;if(isProceduralRenderer(renderer)){var cleanupFn=renderer.listen(native,eventName,listener);(cleanup||(cleanup=currentView.cleanup=[])).push(cleanupFn,null)}else native.addEventListener(eventName,listener,useCapture),(cleanup||(cleanup=currentView.cleanup=[])).push(eventName,native,listener,useCapture);var tNode=node.tNode;void 0===tNode.outputs&&(tNode.outputs=generatePropertyAliases(node.flags,1));var outputData,outputs=tNode.outputs;outputs&&(outputData=outputs[eventName])&&createOutput(outputData,listener)}function createOutput(outputs,listener){for(var i=0;i<outputs.length;i+=2){ngDevMode&&assertDataInRange(outputs[i]);var subscription=data[outputs[i]][outputs[1|i]].subscribe(listener);cleanup.push(subscription.unsubscribe,subscription)}}function elementEnd(){isParent?isParent=!1:(ngDevMode&&assertHasParent(),previousOrParentNode=previousOrParentNode.parent),ngDevMode&&assertNodeType(previousOrParentNode,3);var queries=previousOrParentNode.queries;queries&&queries.addNode(previousOrParentNode),queueLifecycleHooks(previousOrParentNode.flags,currentView)}function elementAttribute(index,name,value){if(value!==NO_CHANGE){var element=data[index];null==value?isProceduralRenderer(renderer)?renderer.removeAttribute(element.native,name):element.native.removeAttribute(name):isProceduralRenderer(renderer)?renderer.setAttribute(element.native,name,stringify$1(value)):element.native.setAttribute(name,stringify$1(value))}}function elementProperty(index,propName,value){if(value!==NO_CHANGE){var node=data[index],tNode=node.tNode;void 0===tNode.inputs&&(tNode.inputs=generatePropertyAliases(node.flags,0));var dataValue,inputData=tNode.inputs;if(inputData&&(dataValue=inputData[propName]))setInputsForProperty(dataValue,value);else{var native=node.native;isProceduralRenderer(renderer)?renderer.setProperty(native,propName,value):native.setProperty?native.setProperty(propName,value):native[propName]=value}}}function createTNode(tagName,attrs,data,localName){return{tagName:tagName,attrs:attrs,localNames:localName?[localName,-1]:null,initialInputs:void 0,inputs:void 0,outputs:void 0,data:data}}function setInputsForProperty(inputs,value){for(var i=0;i<inputs.length;i+=2)ngDevMode&&assertDataInRange(inputs[i]),data[inputs[i]][inputs[1|i]]=value}function generatePropertyAliases(lNodeFlags,direction){var size=(4092&lNodeFlags)>>2,propStore=null;if(size>0)for(var start=lNodeFlags>>12,isInput=0===direction,i=start,ii=start+size;i<ii;i++){var directiveDef=tData[i],propertyAliasMap=isInput?directiveDef.inputs:directiveDef.outputs;for(var publicName in propertyAliasMap)if(propertyAliasMap.hasOwnProperty(publicName)){propStore=propStore||{};var internalName=propertyAliasMap[publicName],hasProperty=propStore.hasOwnProperty(publicName);hasProperty?propStore[publicName].push(i,internalName):propStore[publicName]=[i,internalName]}}return propStore}function elementStyle(index,styleName,value,suffix){if(value!==NO_CHANGE){var lElement=data[index];if(null==value)isProceduralRenderer(renderer)?renderer.removeStyle(lElement.native,styleName,RendererStyleFlags3.DashCase):lElement.native.style.removeProperty(styleName);else{var strValue=suffix?stringify$1(value)+suffix:stringify$1(value);isProceduralRenderer(renderer)?renderer.setStyle(lElement.native,styleName,strValue,RendererStyleFlags3.DashCase):lElement.native.style.setProperty(styleName,strValue)}}}function text(index,value){ngDevMode&&assertNull(currentView.bindingStartIndex,"text nodes should be created before bindings");var textNode=null!=value?isProceduralRenderer(renderer)?renderer.createText(stringify$1(value)):renderer.createTextNode(stringify$1(value)):null,node=createLNode(index,3,textNode);isParent=!1,appendChild(node.parent,textNode,currentView)}function textBinding(index,value){ngDevMode&&assertDataInRange(index);var existingNode=data[index];ngDevMode&&assertNotNull(existingNode,"existing node"),existingNode.native?value!==NO_CHANGE&&(isProceduralRenderer(renderer)?renderer.setValue(existingNode.native,stringify$1(value)):existingNode.native.textContent=stringify$1(value)):(existingNode.native=isProceduralRenderer(renderer)?renderer.createText(stringify$1(value)):renderer.createTextNode(stringify$1(value)),insertChild(existingNode,currentView))}function directiveCreate(index,directive,directiveDef,queryName){var instance;ngDevMode&&assertNull(currentView.bindingStartIndex,"directives should be created before any bindings"),ngDevMode&&assertPreviousIsParent();var flags=previousOrParentNode.flags;if(0==(4092&flags)?flags=index<<12|4|3&flags:flags+=4,previousOrParentNode.flags=flags,ngDevMode&&assertDataInRange(index-1),Object.defineProperty(directive,NG_HOST_SYMBOL,{enumerable:!1,value:previousOrParentNode}),data[index]=instance=directive,index>=tData.length&&(tData[index]=directiveDef,queryName)){ngDevMode&&assertNotNull(previousOrParentNode.tNode,"previousOrParentNode.tNode");var tNode_1=previousOrParentNode.tNode;(tNode_1.localNames||(tNode_1.localNames=[])).push(queryName,index)}var diPublic=directiveDef.diPublic;diPublic&&diPublic(directiveDef),null!=directiveDef.attributes&&3==(3&previousOrParentNode.flags)&&setUpAttributes(previousOrParentNode.native,directiveDef.attributes);var tNode=previousOrParentNode.tNode;return tNode&&tNode.attrs&&setInputsFromAttrs(instance,directiveDef.inputs,tNode),queueInitHooks(index,directiveDef.onInit,directiveDef.doCheck,currentView.tView),instance}function setInputsFromAttrs(instance,inputs,tNode){var directiveIndex=((4092&previousOrParentNode.flags)>>2)-1,initialInputData=tNode.initialInputs;(void 0===initialInputData||directiveIndex>=initialInputData.length)&&(initialInputData=generateInitialInputs(directiveIndex,inputs,tNode));var initialInputs=initialInputData[directiveIndex];if(initialInputs)for(var i=0;i<initialInputs.length;i+=2)instance[initialInputs[i]]=initialInputs[1|i]}function generateInitialInputs(directiveIndex,inputs,tNode){var initialInputData=tNode.initialInputs||(tNode.initialInputs=[]);initialInputData[directiveIndex]=null;for(var attrs=tNode.attrs,i=0;i<attrs.length;i+=2){var attrName=attrs[i],minifiedInputName=inputs[attrName];if(void 0!==minifiedInputName){(initialInputData[directiveIndex]||(initialInputData[directiveIndex]=[])).push(minifiedInputName,attrs[1|i])}}return initialInputData}function container(index,directiveTypes,template,tagName,attrs,localRefs){ngDevMode&&assertNull(currentView.bindingStartIndex,"container nodes should be created before any bindings");var currentParent=isParent?previousOrParentNode:previousOrParentNode.parent;ngDevMode&&assertNotNull(currentParent,"containers should have a parent");var lContainer={views:[],nextIndex:0,renderParent:canInsertNativeNode(currentParent,currentView)?currentParent:null,template:null==template?null:template,next:null,parent:currentView,dynamicViewCount:0,queries:null},node=createLNode(index,0,void 0,lContainer);if(null==node.tNode){var queryName=hack_findQueryName(null,localRefs,"");node.tNode=tData[index]=createTNode(tagName||null,attrs||null,[],queryName||null)}addToViewTree(node.data),hack_declareDirectives(index,directiveTypes,localRefs),isParent=!1,ngDevMode&&assertNodeType(previousOrParentNode,0);var queries=node.queries;queries&&(queries.addNode(node),lContainer.queries=queries.container())}function containerRefreshStart(index){ngDevMode&&assertDataInRange(index),previousOrParentNode=data[index],ngDevMode&&assertNodeType(previousOrParentNode,0),isParent=!0,previousOrParentNode.data.nextIndex=0,ngDevMode&&assertSame(previousOrParentNode.native,void 0,"the container's native element should not have been set yet."),executeInitHooks(currentView,currentView.tView,creationMode)}function containerRefreshEnd(){isParent?isParent=!1:(ngDevMode&&assertNodeType(previousOrParentNode,2),ngDevMode&&assertHasParent(),previousOrParentNode=previousOrParentNode.parent),ngDevMode&&assertNodeType(previousOrParentNode,0);var container=previousOrParentNode;container.native=void 0,ngDevMode&&assertNodeType(container,0);for(var nextIndex=container.data.nextIndex;nextIndex<container.data.views.length;)removeView(container,nextIndex)}function refreshDynamicChildren(){for(var current=currentView.child;null!==current;current=current.next)if(0!==current.dynamicViewCount&&current.views)for(var container_1=current,i=0;i<container_1.views.length;i++){var view=container_1.views[i];renderEmbeddedTemplate(view,view.data.template,view.data.context,renderer)}}function embeddedViewStart(viewBlockId){var container=isParent?previousOrParentNode:previousOrParentNode.parent;ngDevMode&&assertNodeType(container,0);var lContainer=container.data,views=lContainer.views,existingView=!creationMode&&lContainer.nextIndex<views.length&&views[lContainer.nextIndex],viewUpdateMode=existingView&&viewBlockId===existingView.data.id;if(viewUpdateMode)previousOrParentNode=views[lContainer.nextIndex++],ngDevMode&&assertNodeType(previousOrParentNode,2),isParent=!0,enterView(existingView.data,previousOrParentNode);else{var newView=createLView(viewBlockId,renderer,getOrCreateEmbeddedTView(viewBlockId,container),null,null);lContainer.queries&&(newView.queries=lContainer.queries.enterView(lContainer.nextIndex)),enterView(newView,createLNode(null,2,null,newView)),lContainer.nextIndex++}return!viewUpdateMode}function getOrCreateEmbeddedTView(viewIndex,parent){ngDevMode&&assertNodeType(parent,0);var tContainer=parent.tNode.data;return(viewIndex>=tContainer.length||null==tContainer[viewIndex])&&(tContainer[viewIndex]=createTView()),tContainer[viewIndex]}function embeddedViewEnd(){isParent=!1;var viewNode=previousOrParentNode=currentView.node,container=previousOrParentNode.parent;if(container){ngDevMode&&assertNodeType(viewNode,2),ngDevMode&&assertNodeType(container,0);var containerState=container.data,previousView=containerState.nextIndex<=containerState.views.length?containerState.views[containerState.nextIndex-1]:null;(null==previousView||previousView.data.id!==viewNode.data.id)&&insertView(container,viewNode,containerState.nextIndex-1)}leaveView(currentView.parent),ngDevMode&&assertEqual(isParent,!1,"isParent"),ngDevMode&&assertNodeType(previousOrParentNode,2)}function componentRefresh(directiveIndex,elementIndex){executeInitHooks(currentView,currentView.tView,creationMode),executeContentHooks(currentView,currentView.tView,creationMode);var template=tData[directiveIndex].template;if(null!=template){ngDevMode&&assertDataInRange(elementIndex);var element=data[elementIndex];ngDevMode&&assertNodeType(element,3),ngDevMode&&assertNotNull(element.data,"Component's host node should have an LView attached."),ngDevMode&&assertDataInRange(directiveIndex);var directive=getDirectiveInstance(data[directiveIndex]),hostView=element.data,oldView=enterView(hostView,element);try{template(directive,creationMode)}finally{refreshDynamicChildren(),leaveView(oldView)}}}function projectionDef(index,selectors){for(var noOfNodeBuckets=selectors?selectors.length+1:1,distributedNodes=new Array(noOfNodeBuckets),i=0;i<noOfNodeBuckets;i++)distributedNodes[i]=[];for(var componentNode=findComponentHost(currentView),componentChild=componentNode.child;null!==componentChild;){if(selectors&&componentChild.tNode){distributedNodes[matchingSelectorIndex(componentChild.tNode,selectors)].push(componentChild)}else distributedNodes[0].push(componentChild);componentChild=componentChild.next}ngDevMode&&assertDataNext(index),data[index]=distributedNodes}function appendToProjectionNode(projectionNode,appendedFirst,appendedLast){if(ngDevMode&&assertEqual(!!appendedFirst,!!appendedLast,"appendedFirst can be null if and only if appendedLast is also null"),appendedLast){var projectionNodeData=projectionNode.data;projectionNodeData.tail?projectionNodeData.tail.pNextOrParent=appendedFirst:projectionNodeData.head=appendedFirst,projectionNodeData.tail=appendedLast,appendedLast.pNextOrParent=projectionNode}}function projection(nodeIndex,localIndex,selectorIndex,attrs){void 0===selectorIndex&&(selectorIndex=0);var node=createLNode(nodeIndex,1,null,{head:null,tail:null});null==node.tNode&&(node.tNode=createTNode(null,attrs||null,null,null)),isParent=!1;for(var currentParent=node.parent,componentNode=findComponentHost(currentView),nodesForSelector=componentNode.data.data[localIndex][selectorIndex],i=0;i<nodesForSelector.length;i++){var nodeToProject=nodesForSelector[i];if(1==(3&nodeToProject.flags)){var previouslyProjected=nodeToProject.data;appendToProjectionNode(node,previouslyProjected.head,previouslyProjected.tail)}else appendToProjectionNode(node,nodeToProject,nodeToProject)}if(canInsertNativeNode(currentParent,currentView))for(var nodeToProject=node.data.head,lastNodeToProject=node.data.tail;nodeToProject;)appendProjectedNode(nodeToProject,currentParent,currentView),nodeToProject=nodeToProject===lastNodeToProject?null:nodeToProject.pNextOrParent}function findComponentHost(lView){for(var viewRootLNode=lView.node;2==(3&viewRootLNode.flags);)ngDevMode&&assertNotNull(lView.parent,"lView.parent"),lView=lView.parent,viewRootLNode=lView.node;return ngDevMode&&assertNodeType(viewRootLNode,3),ngDevMode&&assertNotNull(viewRootLNode.data,"node.data"),viewRootLNode}function addToViewTree(state){return currentView.tail?currentView.tail.next=state:currentView.child=state,currentView.tail=state,state}function initBindings(){ngDevMode&&assertEqual(creationMode,!0,"should only be called in creationMode for performance reasons"),null==currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length)}function bind(value){if(creationMode)return initBindings(),data[bindingIndex++]=value;var changed=value!==NO_CHANGE&&isDifferent(data[bindingIndex],value);return changed&&(data[bindingIndex]=value),bindingIndex++,changed?value:NO_CHANGE}function interpolationV(values){ngDevMode&&assertLessThan(2,values.length,"should have at least 3 values"),ngDevMode&&assertEqual(values.length%2,1,"should have an odd number of values");for(var different=!1,i=1;i<values.length;i+=2)bindingUpdated(values[i])&&(different=!0);if(!different)return NO_CHANGE;for(var content=values[0],i=1;i<values.length;i+=2)content+=stringify$1(values[i])+values[i+1];return content}function interpolation1(prefix,v0,suffix){return bindingUpdated(v0)?prefix+stringify$1(v0)+suffix:NO_CHANGE}function interpolation2(prefix,v0,i0,v1,suffix){return bindingUpdated2(v0,v1)?prefix+stringify$1(v0)+i0+stringify$1(v1)+suffix:NO_CHANGE}function interpolation3(prefix,v0,i0,v1,i1,v2,suffix){var different=bindingUpdated2(v0,v1);return different=bindingUpdated(v2)||different,different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+suffix:NO_CHANGE}function interpolation4(prefix,v0,i0,v1,i1,v2,i2,v3,suffix){return bindingUpdated4(v0,v1,v2,v3)?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+suffix:NO_CHANGE}function interpolation5(prefix,v0,i0,v1,i1,v2,i2,v3,i3,v4,suffix){var different=bindingUpdated4(v0,v1,v2,v3);return different=bindingUpdated(v4)||different,different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+i3+stringify$1(v4)+suffix:NO_CHANGE}function interpolation6(prefix,v0,i0,v1,i1,v2,i2,v3,i3,v4,i4,v5,suffix){var different=bindingUpdated4(v0,v1,v2,v3);return different=bindingUpdated2(v4,v5)||different,different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+i3+stringify$1(v4)+i4+stringify$1(v5)+suffix:NO_CHANGE}function interpolation7(prefix,v0,i0,v1,i1,v2,i2,v3,i3,v4,i4,v5,i5,v6,suffix){var different=bindingUpdated4(v0,v1,v2,v3);return different=bindingUpdated2(v4,v5)||different,different=bindingUpdated(v6)||different,different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+i3+stringify$1(v4)+i4+stringify$1(v5)+i5+stringify$1(v6)+suffix:NO_CHANGE}function interpolation8(prefix,v0,i0,v1,i1,v2,i2,v3,i3,v4,i4,v5,i5,v6,i6,v7,suffix){var different=bindingUpdated4(v0,v1,v2,v3);return different=bindingUpdated4(v4,v5,v6,v7)||different,different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+i3+stringify$1(v4)+i4+stringify$1(v5)+i5+stringify$1(v6)+i6+stringify$1(v7)+suffix:NO_CHANGE}function store(index,value){index>=tData.length&&(tData[index]=null),data[index]=value}function load(index){return ngDevMode&&assertDataInRange(index,data),data[index]}function consumeBinding(){return ngDevMode&&assertDataInRange(bindingIndex),ngDevMode&&assertNotEqual(data[bindingIndex],NO_CHANGE,"Stored value should never be NO_CHANGE."),data[bindingIndex++]}function bindingUpdated(value){return ngDevMode&&assertNotEqual(value,NO_CHANGE,"Incoming value should never be NO_CHANGE."),creationMode||isDifferent(data[bindingIndex],value)?(creationMode&&initBindings(),data[bindingIndex++]=value,!0):(bindingIndex++,!1)}function checkAndUpdateBinding$1(value){return bindingUpdated(value),value}function bindingUpdated2(exp1,exp2){var different=bindingUpdated(exp1);return bindingUpdated(exp2)||different}function bindingUpdated4(exp1,exp2,exp3,exp4){var different=bindingUpdated2(exp1,exp2);return bindingUpdated2(exp3,exp4)||different}function getDirectiveInstance(instanceOrArray){return Array.isArray(instanceOrArray)?instanceOrArray[0]:instanceOrArray}function assertPreviousIsParent(){assertEqual(isParent,!0,"previousOrParentNode should be a parent")}function assertHasParent(){assertNotNull(previousOrParentNode.parent,"previousOrParentNode should have a parent")}function assertDataInRange(index,arr){null==arr&&(arr=data),assertLessThan(index,arr?arr.length:0,"index expected to be a valid data index")}function assertDataNext(index){assertEqual(data.length,index,"index expected to be at the end of data")}function renderComponent(componentType,opts){void 0===opts&&(opts={});var rendererFactory=opts.rendererFactory||domRendererFactory3,componentDef=componentType.ngComponentDef;componentDef.type!=componentType&&(componentDef.type=componentType);var component,hostNode=locateHostElement(rendererFactory,opts.host||componentDef.tag),rootContext={component:null,scheduler:opts.scheduler||requestAnimationFrame,clean:CLEAN_PROMISE},oldView=enterView(createLView(-1,rendererFactory.createRenderer(hostNode,componentDef.rendererType),createTView(),null,rootContext),null);try{hostElement(hostNode,componentDef),component=rootContext.component=getDirectiveInstance(directiveCreate(1,componentDef.n(),componentDef))}finally{leaveView(oldView)}return opts.features&&opts.features.forEach(function(feature){return feature(component,componentDef)}),detectChanges(component),component}function detectChanges(component){var hostNode=_getComponentHostLElementNode(component);ngDevMode&&assertNotNull(hostNode.data,"Component host node should be attached to an LView"),renderComponentOrTemplate(hostNode,hostNode.view,component)}function _getComponentHostLElementNode(component){ngDevMode&&assertNotNull(component,"expecting component got null");var lElementNode=component[NG_HOST_SYMBOL];return ngDevMode&&assertNotNull(component,"object is not a component"),lElementNode}function bloomAdd(injector,type){var id=type[NG_ELEMENT_ID];null==id&&(id=type[NG_ELEMENT_ID]=nextNgElementId++);var bloomBit=id%BLOOM_SIZE,mask=1<<bloomBit;bloomBit<64?bloomBit<32?injector.bf0|=mask:injector.bf1|=mask:bloomBit<96?injector.bf2|=mask:injector.bf3|=mask}function getOrCreateNodeInjector(){return ngDevMode&&assertPreviousIsParent(),getOrCreateNodeInjectorForNode(getPreviousOrParentNode())}function getOrCreateNodeInjectorForNode(node){var nodeInjector=node.nodeInjector,parentInjector=node.parent&&node.parent.nodeInjector;return nodeInjector!=parentInjector?nodeInjector:node.nodeInjector={parent:parentInjector,node:node,bf0:0,bf1:0,bf2:0,bf3:0,cbf0:null==parentInjector?0:parentInjector.cbf0|parentInjector.bf0,cbf1:null==parentInjector?0:parentInjector.cbf1|parentInjector.bf1,cbf2:null==parentInjector?0:parentInjector.cbf2|parentInjector.bf2,cbf3:null==parentInjector?0:parentInjector.cbf3|parentInjector.bf3,injector:null,templateRef:null,viewContainerRef:null,elementRef:null}}function createInjectionError(text$$1,token){return new Error("ElementInjector: "+text$$1+" ["+stringify$1(token)+"]")}function diPublicInInjector(di,def){bloomAdd(di,def.type)}function diPublic(def){diPublicInInjector(getOrCreateNodeInjector(),def)}function inject$1(token,flags,defaultValue){return getOrCreateInjectable(getOrCreateNodeInjector(),token,flags,defaultValue)}function injectTemplateRef(){return getOrCreateTemplateRef(getOrCreateNodeInjector())}function injectViewContainerRef(){return getOrCreateContainerRef(getOrCreateNodeInjector())}function getOrCreateInjectable(di,token,flags,defaultValue){var bloomHash=bloomHashBit(token);if(null===bloomHash){var moduleInjector=di.injector;if(!moduleInjector){if(null!=defaultValue)return defaultValue;throw createInjectionError("NotFound",token)}moduleInjector.get(token)}else for(var injector=di;injector&&(injector=bloomFindPossibleInjector(injector,bloomHash));){var node=injector.node,flags_1=node.flags,size=(4092&flags_1)>>2;if(0!==size)for(var start=flags_1>>12,tData=node.view.tView.data,i=start,ii=start+size;i<ii;i++){var directiveDef=tData[i];if(directiveDef.diPublic&&directiveDef.type==token)return getDirectiveInstance(node.view.data[i])}injector=injector.parent}throw createInjectionError("Not found",token)}function bloomHashBit(type){var id=type[NG_ELEMENT_ID];return"number"==typeof id?id%BLOOM_SIZE:null}function bloomFindPossibleInjector(startInjector,bloomBit){for(var mask=1<<bloomBit,injector=startInjector;injector;){var value=bloomBit<64?bloomBit<32?injector.bf0:injector.bf1:bloomBit<96?injector.bf2:injector.bf3;if((value&mask)===mask)return injector;value=bloomBit<64?bloomBit<32?injector.cbf0:injector.cbf1:bloomBit<96?injector.cbf2:injector.cbf3,injector=value&mask?injector.parent:null}return null}function getOrCreateElementRef(di){return di.elementRef||(di.elementRef=new ElementRef$1(0==(3&di.node.flags)?null:di.node.native))}function getOrCreateContainerRef(di){return di.viewContainerRef||(di.viewContainerRef=new ViewContainerRef$1(di.node))}function getOrCreateTemplateRef(di){ngDevMode&&assertNodeType(di.node,0);var data=di.node.data;return di.templateRef||(di.templateRef=new TemplateRef$1(getOrCreateElementRef(di),data.template,getRenderer()))}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function defineComponent(componentDefinition){var type=componentDefinition.type,def={type:type,diPublic:null,n:componentDefinition.factory,tag:componentDefinition.tag||null,template:componentDefinition.template||null,h:componentDefinition.hostBindings||noop$1,attributes:componentDefinition.attributes||null,inputs:invertObject(componentDefinition.inputs),outputs:invertObject(componentDefinition.outputs),methods:invertObject(componentDefinition.methods),rendererType:resolveRendererType2(componentDefinition.rendererType)||null,exportAs:componentDefinition.exportAs,onInit:type.prototype.ngOnInit||null,doCheck:type.prototype.ngDoCheck||null,afterContentInit:type.prototype.ngAfterContentInit||null,afterContentChecked:type.prototype.ngAfterContentChecked||null,afterViewInit:type.prototype.ngAfterViewInit||null,afterViewChecked:type.prototype.ngAfterViewChecked||null,onDestroy:type.prototype.ngOnDestroy||null},feature=componentDefinition.features;return feature&&feature.forEach(function(fn){return fn(def)}),def}function NgOnChangesFeature(definition){function onChangesWrapper(delegateHook){return function(){var simpleChanges=this[PRIVATE_PREFIX];null!=simpleChanges&&(this.ngOnChanges(simpleChanges),this[PRIVATE_PREFIX]=null),delegateHook&&delegateHook.apply(this)}}var inputs=definition.inputs,proto=definition.type.prototype;Object.defineProperty(proto,PRIVATE_PREFIX,{value:void 0,writable:!0});for(var pubKey in inputs)!function(pubKey){var minKey=inputs[pubKey],privateMinKey=PRIVATE_PREFIX+minKey;Object.defineProperty(proto,privateMinKey,{value:void 0,writable:!0});var existingDesc=Object.getOwnPropertyDescriptor(proto,minKey);Object.defineProperty(proto,minKey,{get:function(){return existingDesc&&existingDesc.get?existingDesc.get.call(this):this[privateMinKey]},set:function(value){var simpleChanges=this[PRIVATE_PREFIX],isFirstChange=void 0===simpleChanges;null==simpleChanges&&(simpleChanges=this[PRIVATE_PREFIX]={}),simpleChanges[pubKey]=new SimpleChange(this[privateMinKey],value,isFirstChange),existingDesc&&existingDesc.set?existingDesc.set.call(this,value):this[privateMinKey]=value}})}(pubKey);null!=definition.onInit&&(definition.onInit=onChangesWrapper(definition.onInit)),definition.doCheck=onChangesWrapper(definition.doCheck)}function PublicFeature(definition){definition.diPublic=diPublic}function noop$1(){}function invertObject(obj){if(null==obj)return EMPTY$1;var newObj={};for(var minifiedKey in obj)newObj[obj[minifiedKey]]=minifiedKey;return newObj}function definePipe(_a){_a.type,_a.factory,_a.pure;throw new Error("TODO: implement!")}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function pipe(index,pipeDef,pipe){throw new Error("TODO: implement!")}function pipeBind1(index,v1){throw new Error("TODO: implement!")}function pipeBind2(index,v1,v2){throw new Error("TODO: implement!")}function pipeBind3(index,v1,v2,v3){throw new Error("TODO: implement!")}function pipeBind4(index,v1,v2,v3,v4){throw new Error("TODO: implement!")}function pipeBindV(index,values){throw new Error("TODO: implement!")}function getIdxOfMatchingSelector(tNode,selector){var localNames=tNode.localNames;if(localNames)for(var i=0;i<localNames.length;i+=2)if(localNames[i]===selector)return localNames[i+1];return null}function geIdxOfMatchingDirective(node,type){for(var tData=node.view.tView.data,flags=node.flags,i=flags>>12,ii=i+((4092&flags)>>2);i<ii;i++){var def=tData[i];if(def.diPublic&&def.type===type)return i}return null}function readFromNodeInjector(nodeInjector,node,read,directiveIdx){if(read instanceof ReadFromInjectorFn)return read.read(nodeInjector,node,directiveIdx);var matchingIdx=geIdxOfMatchingDirective(node,read);return null!==matchingIdx?node.view.data[matchingIdx]:null}function add(query,node){for(var nodeInjector=getOrCreateNodeInjectorForNode(node);query;){var predicate=query.predicate,type=predicate.type;if(type){var directiveIdx=geIdxOfMatchingDirective(node,type);if(null!==directiveIdx){var result=readFromNodeInjector(nodeInjector,node,predicate.read||type,directiveIdx);null!==result&&addMatch(query,result)}}else for(var selector=predicate.selector,i=0;i<selector.length;i++){ngDevMode&&assertNotNull(node.tNode,"node.tNode");var directiveIdx=getIdxOfMatchingSelector(node.tNode,selector[i]);if(null!==directiveIdx){ngDevMode&&assertNotNull(predicate.read,"the node should have a predicate");var result=readFromNodeInjector(nodeInjector,node,predicate.read,directiveIdx);null!==result&&addMatch(query,result)}}query=query.next}}function addMatch(query,matchingValue){query.values.push(matchingValue),query.list.setDirty()}function createPredicate(predicate,read){var isArray=Array.isArray(predicate);return{type:isArray?null:predicate,selector:isArray?predicate:null,read:read}}function createQuery$1(previous,queryList,predicate,read){return{next:previous,list:queryList,predicate:createPredicate(predicate,read),values:queryList._valuesTree}}function query(memoryIndex,predicate,descend,read){ngDevMode&&assertPreviousIsParent();var queryList=new QueryList$1;return getCurrentQueries(LQueries_).track(queryList,predicate,descend,read),null!=memoryIndex&&store(memoryIndex,queryList),queryList}function queryRefresh(queryList){var queryListImpl=queryList;return!!queryList.dirty&&(queryList.reset(queryListImpl._valuesTree),queryList.notifyOnChanges(),!0)}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function pureFunction0(pureFn){return getCreationMode()?checkAndUpdateBinding$1(pureFn()):consumeBinding()}function pureFunction1(pureFn,exp){return bindingUpdated(exp)?checkAndUpdateBinding$1(pureFn(exp)):consumeBinding()}function pureFunction2(pureFn,exp1,exp2){return bindingUpdated2(exp1,exp2)?checkAndUpdateBinding$1(pureFn(exp1,exp2)):consumeBinding()}function pureFunction3(pureFn,exp1,exp2,exp3){var different=bindingUpdated2(exp1,exp2);return bindingUpdated(exp3)||different?checkAndUpdateBinding$1(pureFn(exp1,exp2,exp3)):consumeBinding()}function pureFunction4(pureFn,exp1,exp2,exp3,exp4){return bindingUpdated4(exp1,exp2,exp3,exp4)?checkAndUpdateBinding$1(pureFn(exp1,exp2,exp3,exp4)):consumeBinding()}function pureFunction5(pureFn,exp1,exp2,exp3,exp4,exp5){var different=bindingUpdated4(exp1,exp2,exp3,exp4);return bindingUpdated(exp5)||different?checkAndUpdateBinding$1(pureFn(exp1,exp2,exp3,exp4,exp5)):consumeBinding()}function pureFunction6(pureFn,exp1,exp2,exp3,exp4,exp5,exp6){var different=bindingUpdated4(exp1,exp2,exp3,exp4);return bindingUpdated2(exp5,exp6)||different?checkAndUpdateBinding$1(pureFn(exp1,exp2,exp3,exp4,exp5,exp6)):consumeBinding()}function pureFunction7(pureFn,exp1,exp2,exp3,exp4,exp5,exp6,exp7){var different=bindingUpdated4(exp1,exp2,exp3,exp4);return different=bindingUpdated2(exp5,exp6)||different,bindingUpdated(exp7)||different?checkAndUpdateBinding$1(pureFn(exp1,exp2,exp3,exp4,exp5,exp6,exp7)):consumeBinding()}function pureFunction8(pureFn,exp1,exp2,exp3,exp4,exp5,exp6,exp7,exp8){var different=bindingUpdated4(exp1,exp2,exp3,exp4);return bindingUpdated4(exp5,exp6,exp7,exp8)||different?checkAndUpdateBinding$1(pureFn(exp1,exp2,exp3,exp4,exp5,exp6,exp7,exp8)):consumeBinding()}function pureFunctionV(pureFn,exps){for(var different=!1,i=0;i<exps.length;i++)bindingUpdated(exps[i])&&(different=!0);return different?checkAndUpdateBinding$1(pureFn.apply(null,exps)):consumeBinding()}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
